...
def rho_attack(n):
    ...
    k=random.randint(pow(2,32),pow(2,33))
    list_sm3=[]
    test1=SM3()
    for i in range(0,k):
        a=str(m)
        msg=bytearray(a.encode('utf-8'))
        test1.sm3_update(msg)
        h=test1.sm3_final()
        if h[:int(n/4)] in list_sm3:
                        print("攻击成功,攻击长度为:",n,'bit')
                        print("迭代循环次数：",i)
                        return
        list_sm3.append(h[:int(n / 4)])
        m = 2 * m + 1
    print("攻击失败")
